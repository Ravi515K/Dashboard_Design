  ...
  return (
    <Tab.Group>
      <Tab.List className="w-300px border border-black text-center py-2">
        <Tab className={tab ? 'w-[100px] h[100px] border border-black rounded-md  bg-middle-bg':''} onClick={()=>setTab(true)}>
          List Type
        </Tab>
        <Tab
          className={!tab ? 'ml-6 w-[100px] h[100px] border border-black rounded-md bg-middle-bg':'ml-6'} onClick={()=>setTab(false)}
        >
          Card Type
        </Tab>
      </Tab.List>
              ...
              {currentUsers &&
                currentUsers.map((user) => (
                  ...
                  >
                    ...
                    <div className="w-[12vw] p-2 border border-black whitespace-normal">{user.name}</div>
                    <div className="w-[15vw] p-2 border border-black whitespace-normal">{user.email}</div>
                    <div className="w-[16vw] p-2 border border-black whitespace-normal">
                      {user.address.street},{user.address.suite},
                      {user.address.city},{user.address.zipcode}
                    </div>
                    <div className="w-[10vw] p-2 border border-black whitespace-normal">{user.phone}</div>
                    <div className="w-[10vw] p-2 border border-black whitespace-normal">{user.website}</div>
                    <div className="w-[10vw] p-2 border border-black whitespace-normal">{user.company.name}</div>
